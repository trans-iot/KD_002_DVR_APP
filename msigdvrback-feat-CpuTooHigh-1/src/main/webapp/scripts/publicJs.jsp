...
%>
	...
	<%--
	 ...
	 @param title
	 --%>
	function showTagLOV(name, param, title){
		var newUrl = "${ctx}/" + name + "/index.html?" + Math.round(new Date().getTime())+'&'+param;
		var level = getCurrentLevel() + 1;
		window.parent.parent.doShowLOV(newUrl, title, level);
	}
	<%--
	...
	function cwAjaxUpload(uploadForm, callback) {
	    ...
	    $.cw = $.cw || {};
	    $.cw.afterUploaded = function(data) {
            if (data && data.errCde == "00") {
                callback(data.pathInfo);
            } else {
                alert(data.errMsg);
            }
	    };
