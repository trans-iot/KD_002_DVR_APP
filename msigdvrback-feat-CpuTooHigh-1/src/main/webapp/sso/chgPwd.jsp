	...
	<script>
		...
		function doSave() {
			if (/\s/.test($("#renewpwd").val())  || /\s/.test($("#newpwd").val()) || /\s/.test($("#oldpwd").val()) ) {
				<%-- 密碼不可含有空格 --%>
				alert("<spring:message code='pwd.space.label' />");
				return false;
			}
			...
			if (valid) {
				...
				.done(function(data) {
					if(data.result == "OK") {
						...
						<%-- 您的新密碼不可與之前使用過的密碼一樣 --%>
						alert('<spring:message code="newPwd.used.label" />');
					}
				});
			}
		}
...
<body>
	<br/><br/><br/>
	<div id="mainDiv">
		<p style="color: red;text-align: center;"><spring:message code="pwd.ninety.role" /></p>
